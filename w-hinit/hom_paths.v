...
Definition X : WAlg A B := (C; c).
Definition Z : WAlgFib A B X := (E; e).
Definition k : WAlgFibMap A B X Z := (g; p).
Definition l : WAlgFibMap A B X Z := (h; q).

(* We will go through the following chain of equivalent types U_0 ... U_n. *)
Definition U_0 := k = l.

Definition P := fun u : forall w, E w => forall x f, u (c x f) = e x f (fun b => u (f b)).
Definition U_1 := { a : g = h & transport P a p = q }.
...
Definition Q x f := fun u : forall w, E w => u (c x f) = e x f (fun b => u (f b)).
Definition U_2 := { a : g = h &
...
Definition U_3 := { a : g = h & forall x f,
...
Definition M x f := fun u : forall w, E w => u (c x f).
Definition N x f := fun u : forall w, E w => e x f (fun b => u (f b)).
Definition U_4 := { a : g = h & forall x f,
...
Definition U_5 := { a : g = h & forall x f,
  ap (M x f) a = p x f @ ap (N x f) a @ (q x f)^ }.
...
Definition U_6 := { a : g = h & forall x f, apD10 a (c x f) =
...
Definition U_7 := { a : g == h & forall x f, a (c x f) =
  p x f @ ap (e x f) (path_forall _ _ (fun b => a (f b))) @ (q x f)^ }.

Lemma u_6_eq_u_7 : U_6 <~> U_7.
Proof.
  apply symmetry.
...
Lemma u_0_eq_u_7 : k = l <~> WAlgFibCell A B X Z k l.
