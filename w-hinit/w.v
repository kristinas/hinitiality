...
Definition WHom {A} {B} (X Y : WAlg A B) : Type :=
  match X, Y with (C;c), (D;d) =>
    { f : C -> D & forall a t, f (c a t) = d a (fun b => f (t b)) }
  end.
...
forall a t, h (c a t) = e a t (fun b => h (t b))) n p = q }.
  apply symmetry; refine (equiv_path_sigma _ (f;p) (g;q)).  
apply transitivity with { n : f = g & forall a t, apD10 n (c a t) = p a t @
ap (e a t) (path_forall _ _ (fun b => apD10 n (t b))) @ (q a t)^ }.
  apply equiv_functor_sigma_id; path_induction; simpl.
  apply transitivity with (p == q).
    apply symmetry; apply equiv_path_forall.
  apply equiv_functor_forall_id; intro a; apply transitivity with (p a == q a).
    apply symmetry; refine (equiv_path_forall _ _).
  refine (equiv_functor_forall_id _); intro t; rewrite path_forall_1; simpl.
